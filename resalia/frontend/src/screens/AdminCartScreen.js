...
export default function AdminCartScreen() {
  ...
  const [{ cart, currency, error, loading, loadingModal }, dispatch] =
    useReducer(reducer, {
      cart: {},
      currency: '',
      error: '',
  ...
  const loadFileHandler = async (e) => {
    ...
    await axios
      .put('/api/carts/upload-image', imageData, {
        headers: { Authorization: `Bearer ${userInfo.token}` },
      })
      .then((res) => {
  ...
  const createItemHandler = async (e) => {
    ...
    await axios
      .post('/api/carts/createCartItem', formData, {
        headers: { Authorization: `Bearer ${userInfo.token}` },
      })
      .then((res) => {
        dispatch({ type: 'UPDATE_SUCCESS', payload: res.data });
    ...
    <>
      <div>
        ...
        {loading ? (
          ...
          <div>
            <h1 className="mt-4">{cart.name}</h1>
                  ...
                  <div className="profile-pic mb-4">
                    ...
                    <img
                      src={`http://localhost:5000/images/carts/${
                        itemImage || 'cart-item-default.png'
                      }`}
                      alt={itemName}
                      id="output"
                      width="200"
